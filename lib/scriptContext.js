...
function createContext(config, scripts) {
    ...
    if (true /*scripts.canProvide?*/) {
        ...
        const provide = {
            provideFn: (alias, fn) => {
                if (scriptFn[alias]) { throw Error(`${ alias } is already registered`); }
                if (typeof fn !== 'function') { throw Error('supplied value must be function'); }
            ...
            provideEvent: (eventName) => {
                if (scriptEvents[eventName]) { throw Error(`${ eventName} is already registered`); }
